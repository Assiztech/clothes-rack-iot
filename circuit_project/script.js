...
async function fetchTemperature() {
    const response = await fetch('http://127.0.0.1:8000/status' , { method: 'GET' });
...
async function updateTemperature() {
    ...
    lightElement.textContent = `light: ${data.light} °C`;

    const rainElement = document.getElementById('rain');
    rainElement.textContent = 'Loading...';
    rainElement.textContent = `rain: ${data.rain} °C`;

}
async function forceCollect() {
    try {
        const response = await fetch('http://127.0.0.1:8000/force-collect', { method: 'GET' });
        if (response.ok) {
...
async function autoManual() {
    try {
        const response = await fetch('http://127.0.0.1:8000/auto-manual', { method: 'GET' });
        if (response.ok) {
            alert('Action executed successfully');
            updateTemperature();
        } else {
